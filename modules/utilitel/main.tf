...
variable "image18" 		           { default = ["ubuntu/images/hvm-ssd/ubuntu-bionic-18.04-amd64-server-*"] }
...
output "internal_cidr_blocks" { value = ["${var.cidrs[var.public]}","${var.cidrs[var.corporate]}","${var.cidrs[var.ops]}","${var.cidrs[var.control]}","${var.cidrs[var.c2_cidr]}"] }

output "route_tables"         { value = "${module.network.route_tables}" }

output "jumpbox_addresses"      { value = "${module.public.jumpbox_addresses}" }
